...
/usr/bin/time -v /usr/bin/ffmpeg -i "../../${file}" -c:v libx264 -f mov "${path}${file:1}_tc"/"output_std1".mov 2> transcoding_info_std_test1.txt
/usr/bin/time -v /usr/bin/ffmpeg -i "../../${file}" -c:v libx264 -f mov "${path}${file:1}_tc"/"output_std2".mov 2> transcoding_info_std_test2.txt
/usr/bin/time -v /usr/bin/ffmpeg -i "../../${file}" -c:v libx264 -f mov "${path}${file:1}_tc"/"output_std3".mov 2> transcoding_info_std_test3.txt
cd ../../
done
